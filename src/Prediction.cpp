...
void Prediction::updateState(PredictionInfo* information, Collision* collision, int& state, int count)
{
    ...
    if (GameManager::is9BallGame()) {
        state = (collision->firstHitBall->classification == myClassification) && (information[9].originalOnTable != information[9].onTable);
        return;
    }
...
bool Prediction::init()
{
    ...
    if (gBalls->init() && GenericData::get(&genericData)) {
        ...
        powerAmount = getPowerAmount(false);
...
void Prediction::initAutoAim()
{
    ...
    Ball*           ball;
    bool            result;
    vec_t           angle, actualAngle, angleCos, angleSin, powerAmount, time, time2, v29;
    Vector2D        spinAmount;
    GenericData*    genericData;
    PredictionInfo* predictionInfo;
    ...
    if (gBalls->init() && GenericData::get(&genericData)) {
        count = gBalls->getCount();
        spinAmount = getSpinAmount();
        powerAmount = getPowerAmount(true);
    ...
    Label:
        ...
        if (angle <= maxAngle && this->stateB == false) {
            angle += gTimePerTick;
            goto Label;
        }
...
vec_t Prediction::getPowerAmount(bool force)
{
    vec_t  power, maxPower;
    SIZE_T gameManager;
    ...
    power = 1.0 - sqrt(1.0 - power * maxPower / maxPower);
    power *= maxPower;
