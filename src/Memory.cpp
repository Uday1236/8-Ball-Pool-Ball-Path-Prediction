...
HANDLE Memory::getProcessHandle()
{
    ...
    if (processHandle == nullptr) {
        ...
        processHandle = OpenProcess(PROCESS_ALL_ACCESS, FALSE, static_cast<SIZE_T>(processID));
        if (processHandle == nullptr) {
...
BOOL Memory::read(SIZE_T address, PVOID buffer, SIZE_T bufferSize)
{
    ...
    return ReadProcessMemory(processHandle, reinterpret_cast<PVOID>(address), buffer, bufferSize, &writtenBytes);
}
...
BOOL Memory::write(SIZE_T address, PVOID buffer, SIZE_T bufferSize)
{
    ...
    return WriteProcessMemory(processHandle, reinterpret_cast<PVOID>(address), buffer, bufferSize, &writtenBytes);
}

DWORD Memory::getGameModuleBase()
{
